...
shinyUI(pageWithSidebar(
  ...
  sidebarPanel(
    h3("Upload Data"),
    fluid = TRUE,
    fluidRow(fileInput('file1', 'Choose CSV File',
                                                    accept=c('text/csv', 
    ...
    tabsetPanel(
      ...
      tabPanel("Routing", 
               h5("NAM Parameters"),
               fluidRow(
               column(4,numericInput("kval", "K:", 3.0, min = 1., max = 1000.),),
               column(4,numericInput("xval", "X:", 0.2, min = 0.01, max = 50.)),
               column(4,numericInput("bedwith", "Bed With:", 10., min = 0.1, max = 1000.)),
               column(4,numericInput("sslope", "Side Slope:", 2., min = 0.1, max = 10.)),
               column(4,numericInput("cslope", "Channel Slope:", 0.001, min = 0.000001, max = 0.5)),
               column(4,numericInput("manning", "Roughness :", 0.01, min = 0.00001, max = 1.)),
               column(4,numericInput("rlength", "River Length :", 15., min = 0.1, max = 1000.))
  ...
  mainPanel(
    tabsetPanel(type = "tabs",
                ...
                tabPanel("Routing", plotlyOutput("routing",height = "800px"))
