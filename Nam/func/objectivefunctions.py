...
def peak_(evaluation, simulation):
    if len(evaluation) == len(simulation):
        obs, sim = np.array(evaluation), np.array(simulation)
        mean_obs = np.nanmean(obs)
        mean_sim = np.nanmean(sim)
        ret = np.sqrt(np.nansum(((obs + mean_obs) * ((obs - sim) ** 2) / (2 * mean_obs))) / len(sim))
        return float(ret)
    else:
        # logging.warning("evaluation and simulation lists does not have the same length.")
        return np.nan
...
def low_(evaluation, simulation):
    if len(evaluation) == len(simulation):
        obs, sim = np.array(evaluation), np.array(simulation)
        mean_obs = np.nanmean(obs)
        mean_sim = np.nanmean(sim)
        A = (np.nanmin(obs) - mean_obs) * (obs - mean_obs)
        B = 2 * mean_obs * (np.nanmax(obs) - mean_obs)
        C = ((obs - sim) ** 2)
        D = np.nansum((obs - mean_obs) ** 2)
        ret = np.nansum(((A / B) + 1) * C) / D
        return float(ret)
    else:
        # logging.warning("evaluation and simulation lists does not have the same length.")
        return np.nan
